...
def save_excel(local_key, local_value, change_key, change_value, asymptomatic_key, asymptomatic_value, filename):
    # 创建一个新的Excel
...
if __name__ == '__main__':
    ...
    for file in dirs:
        ...
        text = f.read()
        local_data = refine_dict(get_local(text))
        ...
        save_excel(local_data_key, local_data_value, change_data_key, change_data_value, asymptomatic_data_key, asymptomatic_data_value, file)
        f.close()
        print(f"{file}提取完成")
    print("全部提取完毕")
